...
function Parse-AccountInfo {
    ...
    $sessionReset = ""
    $weeklyReset  = ""
    $countdownMatches = [regex]::Matches($Text, 'Resets in ([\d\w\s]+(?:hr|min)[^\n\r]*)')
    if ($countdownMatches.Count -ge 1) {
        $sessionReset = "Resets in $($countdownMatches[0].Groups[1].Value.Trim())"
    }
    if ($countdownMatches.Count -ge 2) {
        $weeklyReset = "Resets in $($countdownMatches[1].Groups[1].Value.Trim())"
    }
    if (-not $weeklyReset) {
        $otherResets = [regex]::Matches($Text, 'Resets (?!in\b)([^\n\r]+)')
        if ($otherResets.Count -ge 1) {
            $weeklyReset = "Resets $($otherResets[0].Groups[1].Value.Trim())"
        }
    }
